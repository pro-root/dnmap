...
# CHANGELOG
# 0.7
#  - Thanks to disclosure@d1b.org for the followin: Fixed path traversal from the client, attacking the server.
#  - Added colors to the output, so it is easier to analize a large list of clients.
#  - Fixed a bug with root permisions detection.
#  - Now if the nmap comamdn fails in the client, we alert the user and store the failed command for later use (normally due to nmap not being executed as root)
#  - Now we check for new commands on the file every 5 seconds
#  - We print the amount of commands in the queue to send
#
# 0.6.1
#  - some minor text messages fixes (Thanks to vero.valeros@gmail.com)
#
# 0.6
...
#
# TODO
# - Ban out clients based on their IP address
# - If the client is processing a command and the servers goes down, we lost that command!
# - Client authentication / registration?
# - Sort the stats by some field
# - Show the ETA
# - Show progress status
# - check the insert
# - limit the size of the file receibed by the server!! so nobody can fill our disk...
# - it seems that for some reason some clients are NOT deleted after the timeout!!!
...
def usage():
  ...
  print "  -p, --port        TCP port where we listen for connections. By default 46001, so vanilla nmap does not catch us."
  print "  -L, --log-file        Log file name. Defaults to /var/log/dnmap_server.conf. We always generate a log."
  print "  -l, --log-level       Log level. Defaults to info."
  print "  -v, --verbose_level         Verbose level. Give a number between 1 and 5. Defaults to 2. Level 0 means be quiet."
  print "  -t, --client-timeout         How many time should we wait before marking a client Offline. We still remember its values just in case it cames back. Specified in seconds, defaults to 3600"
  ...
  print "  -c, --not-colors         Do not use colors for printing the information."
  print
...
def show_info():
	...
	try:
		...
		if verbose_level > 0:
			if usecolors:
				line = '\x1b\x5b1;33;40m=| MET:{0} | Amount of Online clients: {1} | Commands queue: {2} |=\x1b\x5b0;0;40m'.format(diff_time, amount,commands_queue_len)
			else:
				line = '=| MET:{0} | Amount of Online clients: {1} | Commands queue: {2} |='.format(diff_time, amount,commands_queue_len)
		...
		if clients != {}:
			if verbose_level > 1:
				...
				mlog.info(line)
				if usecolors:
					line = '\x1b\x5b1;35;40m{0:15}\t{1}\t{2}\t{3}\t{4}\t\t{5}\t{6}\t{7}\t{8}\t{9}\x1b\x5b0;0;40m'.format('Client Alias','#Commands','Last Time Seen', '(time ago)', 'UpTime', 'Version', 'IsRoot', 'RunCmdXMin', 'AvrCmdXMin', 'Status')
				else:
				...
				# Alternate client line colors so it is easier to visualize a large list
				color_list = ['32', '36']
				for i in clients:
					if clients[i]['Status'] != 'Offline':
						...
						if usecolors:
							line = '\x1b\x5b1;'+color_list[clients.keys().index(i) % 2]+';40m{0:15}\t{1}\t\t{2}({3:2d}\'{4:2d}\")\t{5:2d}h{6:2d}m\t\t{7}\t{8}\t{9:10.1f}\t{10:9.1f}\t{11}\x1b\x5b0;0;40m'.format(clients[i]['Alias'], clients[i]['NbrCommands'], lasttime, time_diff_mins, time_diff_secs, uptime_diff_hours, uptime_diff_mins , clients[i]['Version'], clients[i]['IsRoot'], clients[i]['RunCmdsxMin'], clients[i]['AvrCmdsxMin'], clients[i]['Status'])
						else:
...
def check_clean(line):
	""" Check for unwanted characters in the line """
	global debug
	try:
		outbound_chars = [';', '#', '`', '..', '/']
		ret = True
		for char in outbound_chars:
			if char in line:
				if verbose_level > 4:
					print '\tInvalid char {0} found in line {1}'.format(char,line)
				ret = False
		return ret
...
def process_input_line(data,ourtransport,client_id):
	...
	try:
		...
		if 'Starts the Client ID:' in data:
			...
			received_nmap_output_file = data.split(':')[1].strip('\n')
			# We check the nmap file name input...
			if not check_clean(received_nmap_output_file):
				# Nmap output file had invalid characters...
				nmap_output_file = 'nmap_results/'+str(random.randrange(0, 100000000, 1))+'.nmap'
				msgline = 'WARNING. The nmap file name sent by the client {0}({1}) had invalid chars. Generating the random file name {2}'.format(alias, client_id,nmap_output_file)
				log.msg(msgline, logLevel=logging.INFO)
				if verbose_level > 1:
					print '+ '+msgline
			else:
				nmap_output_file = 'nmap_results/'+received_nmap_output_file+'.nmap'

			if verbose_level > 2:
				log.msg('\tNmap output file is: {0}'.format(nmap_output_file), logLevel=logging.DEBUG)

			output_file_descriptor = open(nmap_output_file, 'a+')
			...
			if data != '':
				...
				# In the future we plan to re-scheadule the command for root only clients.
				if verbose_level > 1:
					print 'WARNING! Nmap data received was empty. This usually means nmap failed to execute the command. Storing the failed command for later analysis in file failed_nmap_commands.txt'
				file = open('failed_nmap_commands.txt','a+')
				file.writelines(clients[client_id]['LastCommand'])
				file.flush
				file.close()
...
def process_nmap_commands(logger_name):
	...
	loop2.start(5.0) # call every second
...
def main():
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], "cf:l:L:p:P:s:t:v:", ["not-colors","nmap-commands=","log-level=","log-server=","port=","pem-file=", "sort-type=","client-timeout=","verbose-level="])
	...
	for opt, arg in opts:
	    ...
	    if opt in ("-c", "--not-colors"): usecolors=False
